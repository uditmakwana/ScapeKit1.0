...
{
    internal class Utility : ScriptableObject
    {
        private static Dictionary<string, Texture2D> iconCache = new Dictionary<string, Texture2D>();

        internal static Texture2D GetIcon(string filename, string filename2x = "", bool invertColors = false)
        {
            ...
            NPath resourcesPath = Directory.GetCurrentDirectory().ToNPath().Combine("Assets/ScapeKitPlugin/Editor/Resources/", filename);
            MemoryStream stream = new MemoryStream(resourcesPath.ReadAllBytes());
    ...
    static class StreamExtensions
    {
        ...
        static StreamExtensions()
        {
            ...
            if (loadImage == null)
            {
                UnityEngine.Debug.LogError("Could not find ImageConversion.LoadImage method");
            }
        }

        internal static Texture2D ToTexture2D(this Stream input)
        {
        ...
        internal static void InvertColors(this Texture2D texture)
        {
